...
impl Val {
    ...
    pub(crate) fn into_table_element(
        ...
        match (self, ty) {
            (Val::FuncRef(Some(f)), ValType::Ref(RefType { heap_type: HeapType::Func, .. })) => {
            ...
            (Val::FuncRef(None), ValType::Ref(RefType { heap_type: HeapType::Func, .. })) => Ok(TableElement::FuncRef(ptr::null_mut())),
            (Val::ExternRef(Some(x)), ValType::Ref(RefType { heap_type: HeapType::Extern, .. })) => {
                Ok(TableElement::ExternRef(Some(x.inner)))
            }
            (Val::ExternRef(None), ValType::Ref(RefType { heap_type: HeapType::Extern, .. })) => Ok(TableElement::ExternRef(None)),
